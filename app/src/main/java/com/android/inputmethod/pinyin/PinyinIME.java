...
//    Log.i("szj", "processKey keycode=" + keyCode + ",realAction=" + realAction);
...
//    Log.i("szj", "responseSoftKeyEvent keyCode=" + keyCode);
  ...
  private void commitChineseResult() {
    if (mInputModeSwitcher.isChineseText()) {
      if (mImeState == ImeState.STATE_COMPOSING/* && !mInputModeSwitcher.isEnterNoramlState()*/) {
        if (!mDecInfo.isCandidatesListEmpty()) {
          commitResultText(mDecInfo.getCurrentFullSent(mCandidatesContainer.getActiveCandiatePos()));
        } else {
          commitResultText(mDecInfo.getComposingStr());
